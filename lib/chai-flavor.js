...
define(['./promise-testing.js','./promise-testing-utils.js','chai'],
function(PromiseTesting,utils,chai){
    'use strict';
    ...
    ExpectHandler.prototype.execute = function(lastResult,next,ctx){
        ...
        switch(this.type){
            case 'result':
                expect(ctx.type).to.equal('result');
                args = [ctx.result];
                break;
            case 'reject':
                expect(ctx.type).to.equal('reject');
                args = [ctx.reason];
                break;
            default :
                args = this.args;
    ...
    PromiseTesting.prototype.scanChai = function(){
        ...
        ['reject','result','rejected','become','with','rejection'].forEach(function(val){
            self.addProperty(val,utils.noOpHandler);
        });
        ...
        Object.getOwnPropertyNames(proto).forEach(function(propName){
            if(!(self.hasProperty(propName) || excludeNames.test(propName))){
                ...
                if(descriptor.configurable){
                    ...
                    switch(type) {
                        case 'object':
                            self.addProperty(propName,utils.echoHandler);
                            echo++;
                            break;
                        case 'function':
                            self.addProperty(propName,utils.executableEchoHandler);
                            exec++;
                    }
                }
            }
